...
def createConfigurations(configList):
    for x in range (0, 5): #5 possible penalty values
        for y in range (0, 4): #4 possible seperation distances
            ...
            #append reward
            if x == 3 or x == 4:
                configList["reward"].append(HUGE_REWARD)
            else:
            ...
            if x == 0:
                ...
                configList["penalty"].append(HUGE_PENALTY)
            elif x == 4:
                configList["penalty"].append(HUGE_PENALTY2)
...
def main():
    ...
    finalData.to_csv('testTrials.csv')
